...
项目已经配置 PostCSS 自动换算。为避免改变现有 H5 和后台样式，只有文件名以 `.vw.css` 或 `.vw.module.css` 结尾的样式文件会转换。在这类文件中可以直接按 750px 设计稿写：
...
构建后 `200px` 和 `100px` 会分别转换成 `26.666667vw` 和 `13.333333vw`。`1px` 细线会保留为 `1px`，媒体查询中的 px 也不会转换。没有 PostCSS 构建流程的纯静态 HTML/CSS/JS 项目，需要使用上面的公式手动换算。
