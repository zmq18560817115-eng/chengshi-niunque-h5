...
export class AdminContentService {
  ...
  private async assertPublishableAsset(input: ReturnType<typeof validateAssetInput>) { if (input.status !== "PUBLISHED" || input.assetType === "EXTERNAL_LINK") return; const keys = input.assetType === "IMAGE" && input.pages.length ? input.pages.map((page) => page.storageKey) : input.storageKey ? [input.storageKey] : []; if (!keys.length || (await Promise.all(keys.map((key) => getObjectStorage().exists(key)))).some((exists) => !exists)) throw new Error("发布失败：上传文件不存在，请重新选择文件后保存"); }
}
