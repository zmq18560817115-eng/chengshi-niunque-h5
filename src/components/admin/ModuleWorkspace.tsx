      ...
      <section className="workspace-editor"><div className="panel-heading"><div><span>{selection.type === "module" ? "编辑模块" : selection.type === "card" ? "编辑卡片" : "编辑资料"}</span><small>{dirty ? "存在未保存修改" : "当前内容已同步"}</small></div></div>
        ...
        {activeCard && <>
          <form id="card-editor" action={updateCardAction} className="admin-form">
            ...
            <div className="form-section"><h2>排序与状态</h2><label>页面排序<FieldHelp>同一分类内数字越小越靠前。</FieldHelp><input name="sortOrder" type="number" min="0" defaultValue={activeCard.sortOrder}/></label><StatusSelect value={activeCard.contentStatus}/></div>
            <details className="advanced-settings"><summary>高级设置</summary><label>按钮文字<FieldHelp>例如“查看资料”或“查看报告”。</FieldHelp><input name="buttonText" defaultValue={activeCard.buttonText}/></label><label>补充说明<FieldHelp>显示在卡片底部的简短提示，可不填。</FieldHelp><input name="footerNote" defaultValue={activeCard.footerNote ?? ""}/></label></details>
          ...
          <details className="asset-create" open={activeCard.assets.length === 0}>
            <summary>第二步：添加报告资料</summary>
            <div className="asset-type-picker"><button type="button" className={assetType === "IMAGE" ? "is-active" : ""} onClick={() => setAssetType("IMAGE")}>上传图片</button><button type="button" className={assetType === "PDF" ? "is-active" : ""} onClick={() => setAssetType("PDF")}>上传 PDF</button><button type="button" className={assetType === "EXTERNAL_LINK" ? "is-active" : ""} onClick={() => setAssetType("EXTERNAL_LINK")}>添加外部链接</button></div>
            <AssetCreateForm cardId={activeCard.id} type={assetType} nextSortOrder={activeCard.assets.length * 10 + 10}/>
          </details>
        </>}
