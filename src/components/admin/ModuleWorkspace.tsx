...
type Asset = { id: string; title: string; description: string | null; assetType: AssetType; openMode: AssetOpenMode; storageKey: string | null; externalUrl: string | null; sortOrder: number; contentStatus: ContentStatus; pages: Array<{ id: string; pageNumber: number }> };
      ...
      <section className="workspace-editor"><div className="panel-heading"><div><span>{selection.type === "module" ? "编辑模块" : selection.type === "card" ? "编辑卡片" : "编辑资料"}</span><small>{dirty ? "存在未保存修改" : "当前内容已同步"}</small></div></div>
        ...
        {activeCard && <>
          <form id="card-editor" action={updateCardAction} className="admin-form">
            ...
            <div className="form-section"><h2>报告与排序</h2><div className="report-count-summary"><span>前端按钮实时显示</span><strong>{activeCard.assets.filter((asset) => asset.contentStatus === "PUBLISHED").length > 0 ? `查看${activeCard.assets.filter((asset) => asset.contentStatus === "PUBLISHED").length}份报告` : "暂无报告"}</strong><small>每条已发布报告计为 1 份；一份报告可以包含多张图片页。</small></div><label>页面排序<FieldHelp>同一分类内数字越小越靠前。</FieldHelp><input name="sortOrder" type="number" min="0" defaultValue={activeCard.sortOrder}/></label><StatusSelect value={activeCard.contentStatus}/></div>
...
function AssetCreateForm({ cardId, type, nextSortOrder }: { cardId: string; type: AssetType; nextSortOrder: number }) {
  return <form className="mini-form"><input type="hidden" name="reportCardId" value={cardId}/><input type="hidden" name="assetType" value={type}/><input type="hidden" name="status" value="DRAFT"/><input type="hidden" name="openMode" value="SAME_TAB"/><label>报告名称<input name="title" required placeholder="例如：营养成分检测报告"/></label><label>选择报告图片<FieldHelp>一次选择的多张图片共同组成 1 份报告；按文件选择顺序显示，单张不超过 20MB，最多 50 张。</FieldHelp><input name="files" type="file" accept="image/jpeg,image/png,image/webp,image/gif" multiple required/></label><label>报告排序<input name="sortOrder" type="number" min="0" defaultValue={nextSortOrder} required/></label><div className="asset-create-actions"><button className="button button-secondary" formAction={createAssetAction}>保存报告草稿</button><button className="button button-primary" formAction={createAndPublishAssetAction}>发布报告并上线卡片</button></div><p className="mini-form-hint">卡片按钮数量由已发布报告数自动生成，不需要手动填写。</p></form>;
}
