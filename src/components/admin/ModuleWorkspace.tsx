...
import { createAssetAction, createCardAction, deleteAssetAction, deleteCardAction, deleteModuleAction, updateAssetAction, updateCardAction, updateModuleAction } from "@/app/admin/actions";
...
export function ModuleWorkspace({ initialModule, publishedModules, moduleOrders, saved }: { initialModule: Module; publishedModules: PublicModule[]; moduleOrders: Array<{ id: string; sortOrder: number }>; saved?: boolean }) {
  ...
  return <div className="workspace-page"><div className="workspace-heading"><div><Link href="/admin/modules">← 返回内容模块</Link><div><h1>{initialModule.title}</h1><span className={`status-badge status-${initialModule.contentStatus.toLowerCase()}`}>{statusText[initialModule.contentStatus]}</span></div><p>在一个工作台中完成内容结构、编辑和预览。</p></div><div className="workspace-view-switch"><button className="button button-secondary" onClick={() => setTreeOpen((value) => !value)}>内容结构</button><a className="button button-secondary" href="#preview">查看预览</a></div></div>
    ...
    <div className="workspace" onInput={previewInput}>
      <aside className={`workspace-tree ${treeOpen ? "is-open" : ""}`}><div className="panel-heading"><div><span>内容结构</span><small>点击节点切换编辑对象</small></div><button aria-label="关闭内容结构" onClick={() => setTreeOpen(false)}>×</button></div><button className={`tree-module ${selection.type === "module" ? "is-active" : ""}`} onClick={() => choose({ type: "module" })}><span>模块</span><strong>{initialModule.title}</strong></button><ol className="tree-list">{initialModule.cards.map((card, cardIndex) => <li key={card.id}><button className={selection.type === "card" && selection.id === card.id ? "is-active" : ""} onClick={() => choose({ type: "card", id: card.id })}><span>卡片 {cardIndex + 1}</span><strong>{card.title}</strong></button><ul>{card.assets.map((asset) => <li key={asset.id}><button className={selection.type === "asset" && selection.id === asset.id ? "is-active" : ""} onClick={() => choose({ type: "asset", id: asset.id })}><span>{asset.assetType === "PDF" ? "PDF 报告" : asset.assetType === "IMAGE" ? "图片" : "外部链接"}</span>{asset.title}</button></li>)}</ul></li>)}</ol><details className="tree-add"><summary>＋ 添加卡片</summary><form action={createCardAction} className="mini-form"><input type="hidden" name="moduleId" value={initialModule.id}/><input type="hidden" name="status" value="DRAFT"/><input type="hidden" name="sortOrder" value={initialModule.cards.length * 10 + 10}/><label>卡片标题<input name="title" required placeholder="例如：营养成分检测"/></label><button className="button button-primary">创建卡片</button></form></details>{initialModule.cards.length === 0 && <p className="tree-empty">先添加一张卡片，再为卡片配置资料。</p>}</aside>
      <section className="workspace-editor"><div className="panel-heading"><div><span>{selection.type === "module" ? "编辑模块" : selection.type === "card" ? "编辑卡片" : "编辑资料"}</span><small>{dirty ? "存在未保存修改" : "当前内容已同步"}</small></div></div>
        {selection.type === "module" && <form id="module-editor" action={updateModuleAction} className="admin-form"><input type="hidden" name="id" value={initialModule.id}/><div className="form-section"><h2>基本内容</h2><label>模块名称<FieldHelp>显示在 H5 首页的一级标题。</FieldHelp><input name="title" defaultValue={initialModule.title} required maxLength={200}/></label><label>模块说明<FieldHelp>一句话说明本模块包含什么内容。</FieldHelp><textarea name="description" defaultValue={initialModule.description ?? ""}/></label></div><div className="form-section"><h2>排序状态</h2><label>页面排序<FieldHelp>数字越小，在 H5 页面越靠前。</FieldHelp><input name="sortOrder" type="number" min="0" defaultValue={initialModule.sortOrder}/></label><StatusSelect value={initialModule.contentStatus}/></div><details className="advanced-settings"><summary>高级设置</summary><label>内部标识<FieldHelp>用于系统识别，建议使用小写英文和短横线，创建后非必要不要修改。</FieldHelp><input name="slug" defaultValue={initialModule.slug} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required/></label></details><DangerForm action={deleteModuleAction} id={initialModule.id} label="删除模块" message="删除后该模块以及其中的卡片和资料都不会继续在 H5 展示。已有内容会保留为软删除数据。"/></form>}
        {activeCard && <form id="card-editor" action={updateCardAction} className="admin-form"><input type="hidden" name="id" value={activeCard.id}/><input type="hidden" name="moduleId" value={initialModule.id}/><div className="form-section"><h2>基本内容</h2><label>卡片标题<FieldHelp>显示在展开模块中的卡片标题。</FieldHelp><input name="title" defaultValue={activeCard.title} required/></label><label>卡片说明<FieldHelp>用于解释报告内容或检测范围。</FieldHelp><textarea name="description" defaultValue={activeCard.description ?? ""}/></label></div><div className="form-section"><h2>页面展示</h2><label>按钮文字<FieldHelp>例如“查看资料”或“查看报告”。</FieldHelp><input name="buttonText" defaultValue={activeCard.buttonText}/></label><label>补充说明<FieldHelp>显示在卡片底部的简短提示，可不填。</FieldHelp><input name="footerNote" defaultValue={activeCard.footerNote ?? ""}/></label></div><div className="form-section"><h2>排序状态</h2><label>页面排序<FieldHelp>同一模块内数字越小越靠前。</FieldHelp><input name="sortOrder" type="number" min="0" defaultValue={activeCard.sortOrder}/></label><StatusSelect value={activeCard.contentStatus}/></div><details className="asset-create"><summary>＋ 为此卡片添加资料</summary><div className="asset-type-picker"><button type="button" className={assetType === "PDF" ? "is-active" : ""} onClick={() => setAssetType("PDF")}>上传 PDF 报告</button><button type="button" className={assetType === "IMAGE" ? "is-active" : ""} onClick={() => setAssetType("IMAGE")}>上传图片</button><button type="button" className={assetType === "EXTERNAL_LINK" ? "is-active" : ""} onClick={() => setAssetType("EXTERNAL_LINK")}>添加外部链接</button></div><AssetCreateForm cardId={activeCard.id} type={assetType}/></details><DangerForm action={deleteCardAction} id={activeCard.id} parentName="moduleId" parentId={initialModule.id} label="删除卡片" message="删除后该卡片及其资料不会继续在 H5 展示。资料记录仍会保留为软删除数据。"/></form>}
        {activeAsset && activeCardForAsset && <form id="asset-editor" action={updateAssetAction} className="admin-form"><input type="hidden" name="id" value={activeAsset.id}/><input type="hidden" name="reportCardId" value={activeCardForAsset.id}/><input type="hidden" name="assetType" value={activeAsset.assetType}/><div className="form-section"><h2>基本内容</h2><label>资料名称<FieldHelp>让用户知道点击后会看到什么。</FieldHelp><input name="title" defaultValue={activeAsset.title} required/></label><label>资料说明<FieldHelp>可补充报告日期、版本或图片内容。</FieldHelp><textarea name="description" defaultValue={activeAsset.description ?? ""}/></label></div><div className="form-section"><h2>点击行为</h2>{activeAsset.assetType === "EXTERNAL_LINK" ? <label>外部链接地址<FieldHelp>必须是以 http:// 或 https:// 开头的完整网址。</FieldHelp><input name="externalUrl" type="url" defaultValue={activeAsset.externalUrl ?? ""} placeholder="https://example.com/report" required/></label> : <input type="hidden" name="storageKey" value={activeAsset.storageKey ?? ""}/>}<label>打开方式<select name="openMode" defaultValue={activeAsset.openMode}><option value="SAME_TAB">当前页面打开</option><option value="NEW_TAB">新页面打开</option></select></label></div><div className="form-section"><h2>排序状态</h2><label>页面排序<FieldHelp>同一卡片内数字越小越靠前。</FieldHelp><input name="sortOrder" type="number" min="0" defaultValue={activeAsset.sortOrder}/></label><StatusSelect value={activeAsset.contentStatus}/></div>{activeAsset.assetType !== "EXTERNAL_LINK" && <details className="advanced-settings"><summary>高级设置</summary><label>文件存储路径<FieldHelp>上传功能尚未完成，暂用于关联 MinIO 中已存在的文件，例如 reports/2026/example.pdf。</FieldHelp><input name="storageKey" defaultValue={activeAsset.storageKey ?? ""} required/></label></details>}<DangerForm action={deleteAssetAction} id={activeAsset.id} parentName="reportCardId" parentId={activeCardForAsset.id} label="删除资料" message="删除后该资料立即停止在 H5 展示，但不会物理删除对象存储中的文件。"/></form>}
      </section>
    ...
    <div className="workspace-actionbar"><Link className="button button-secondary" href="/admin/modules" onClick={(event) => { if (dirty && !window.confirm("当前有未保存修改，确定退出编辑吗？")) event.preventDefault(); }}>退出编辑</Link><span className={dirty ? "unsaved" : "saved"}>{saving ? "正在保存…" : dirty ? "存在未保存修改" : saved ? "刚刚已保存" : "内容已保存"}</span><button className="button button-secondary" disabled={saving} onClick={() => submitForm("DRAFT")}>保存为草稿</button><a className="button button-secondary" href="#preview">预览</a><button className="button button-primary" disabled={saving} onClick={() => selection.type === "module" ? setCheckOpen(true) : submitForm()}>{selection.type === "module" ? "发布" : "保存更改"}</button></div>
...
function AssetCreateForm({ cardId, type }: { cardId: string; type: AssetType }) {
  return <div className="mini-form"><input type="hidden" name="reportCardId" value={cardId}/><input type="hidden" name="assetType" value={type}/><input type="hidden" name="status" value="DRAFT"/><input type="hidden" name="sortOrder" value="0"/><label>资料名称<input name="title" required placeholder={type === "PDF" ? "例如：营养成分检测报告" : type === "IMAGE" ? "例如：复核流程图" : "例如：国家标准参考"}/></label>{type === "EXTERNAL_LINK" ? <label>外部链接地址<input name="externalUrl" type="url" required placeholder="https://example.com/report"/></label> : <label>文件存储路径<FieldHelp>上传功能尚未完成，请填写 MinIO 中已有文件的路径。</FieldHelp><input name="storageKey" required placeholder={type === "PDF" ? "reports/example.pdf" : "images/example.jpg"}/></label>}<label>打开方式<select name="openMode" defaultValue={type === "EXTERNAL_LINK" ? "NEW_TAB" : "SAME_TAB"}><option value="SAME_TAB">当前页面打开</option><option value="NEW_TAB">新页面打开</option></select></label><button className="button button-primary" formAction={createAssetAction}>创建资料</button></div>;
}
...
function DangerForm({ action, id, parentName, parentId, label, message }: { action: (data: FormData) => void | Promise<void>; id: string; parentName?: string; parentId?: string; label: string; message: string }) {
  return <section className="danger-zone"><h2>危险操作</h2><p>{message}</p><button type="submit" className="button button-danger" formAction={action} name="id" value={id} onClick={(event) => { if (!window.confirm(`${message}\n\n确定继续吗？`)) event.preventDefault(); }}>{label}</button>{parentName && <input type="hidden" name={parentName} value={parentId}/>}</section>;
}
