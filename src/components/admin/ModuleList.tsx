...
export function ModuleList({ modules }: { modules: ModuleRow[] }) {
  ...
  const filtered = useMemo(() => modules.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()) && (status === "ALL" || item.contentStatus === status)), [modules, query, status]);
  return <section className="admin-section" aria-labelledby="module-list-title">
    ...
    {filtered.length === 0 ? <div className="admin-empty"><strong>没有找到符合条件的模块</strong><p>调整搜索条件，或在上方创建新模块。</p></div> : <div className="module-table-wrap"><table className="module-table"><thead><tr><th>模块名称</th><th>状态</th><th>内容数量</th><th>最后修改</th><th>操作</th></tr></thead><tbody>{filtered.map((item, index) => <tr key={item.id}><td><strong>{item.title}</strong></td><td><span className={`status-badge status-${item.contentStatus.toLowerCase()}`}>{statusText[item.contentStatus]}</span></td><td>{item.cardCount} 张卡片 · {item.assetCount} 条资料</td><td>{new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.updatedAt))}</td><td><div className="row-actions"><Link className="button button-primary" href={`/admin/modules/${item.id}`}>编辑内容</Link><Link className="button button-secondary" href={`/admin/modules/${item.id}#preview`}>手机预览</Link><details className="more-menu"><summary>更多操作</summary><div><form action={moveModuleAction}><input type="hidden" name="id" value={item.id}/><button name="direction" value="up" disabled={index === 0}>上移</button><button name="direction" value="down" disabled={index === filtered.length - 1}>下移</button></form></div></details></div></td></tr>)}</tbody></table></div>}
  </section>;
}
