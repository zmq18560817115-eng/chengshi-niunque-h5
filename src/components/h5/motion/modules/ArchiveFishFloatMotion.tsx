...
const fishAssets = [
  { src: "/design/final-v1/motion/archive-runtime/fish-01.png", x: 57, y: 4414, width: 134, height: 76, delay: -120, duration: 2100, amplitude: 8, jitter: 2 },
  { src: "/design/final-v1/motion/archive-runtime/fish-02.png", x: 303, y: 4414, width: 136, height: 70, delay: -780, duration: 2380, amplitude: 6, jitter: 1 },
  { src: "/design/final-v1/motion/archive-runtime/fish-03.png", x: 546, y: 4418, width: 136, height: 69, delay: -430, duration: 2240, amplitude: 9, jitter: 2 },
  { src: "/design/final-v1/motion/archive-runtime/fish-04.png", x: 795, y: 4407, width: 132, height: 84, delay: -1040, duration: 2520, amplitude: 7, jitter: 1 },
] as const;
  ...
  return <div className={`archive-fish-float ${ready ? "is-ready" : ""} ${visible ? "is-visible" : ""} ${fallback ? "is-fallback" : ""}`} data-motion-module="archiveFishFloat" aria-hidden="true">
    ...
    {motionAllowed && <MotionBoundary fallback={staticFish}>
      <MotionStage masterWidth={masterWidth} masterHeight={masterHeight} assets={motionAssets} enabled={enabled} crossfadeMs={0} fallback={staticFish} onStateChange={onStateChange}>
        ...
        {fishAssets.map((fish, index) => <Image key={fish.src} className="archive-fish-float-layer" data-fish-index={index + 1} src={fish.src} alt="" width={fish.width} height={fish.height} unoptimized style={{ ...position(fish), "--archive-fish-delay": `${fish.delay}ms`, "--archive-fish-duration": `${fish.duration}ms`, "--archive-fish-amp": `${fish.amplitude}px`, "--archive-fish-jitter": `${fish.jitter}px` } as CSSProperties} />)}
      </MotionStage>
    </MotionBoundary>}
  </div>;
}
