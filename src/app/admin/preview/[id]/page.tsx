...
export default async function StandaloneAdminPreview({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ width?: string }> }) {
  ...
  const previewModule: PublicModule = { id: module.id, slug: module.slug, title: module.title, description: module.description, cards: module.cards.map((card) => ({ id: card.id, title: card.title, description: card.description, buttonText: card.assets.length ? `查看${card.assets.length}份报告` : "暂无报告", footerNote: card.footerNote, assets: card.assets.map((asset) => ({ id: asset.id, title: asset.title, description: asset.description, type: asset.assetType, href: asset.assetType === "EXTERNAL_LINK" ? asset.externalUrl ?? "" : asset.storageKey ? `/reports/${asset.assetType === "PDF" ? "pdf" : "image"}/${asset.id}` : "", openMode: asset.openMode === "NEW_TAB" ? "new_tab" : "same_tab", pages: asset.assetType === "IMAGE" ? asset.pages.map((page) => ({ id: page.id, pageNumber: page.pageNumber, href: `/reports/image/page/${page.id}` })) : [] })) })) };
