...
`pnpm build:production` 会通过仓库脚本固定启用 standalone 并生成服务器启动产物；不要在 PM2 发布中用普通 `pnpm build` 替代它。初始化命令使用可选 `.env` 文件：仓库目录存在 `.env` 时会读取它，Docker/PM2 已注入生产环境变量且不存在 `.env` 时也不会因缺少文件退出。
...
若目标服务器已经运行宿主机 Nginx 并承载其他域名，不得原样启动本编排中的 `nginx` 服务抢占 80 端口。此时应沿用 PM2 入口，或只启动应用容器并把端口绑定到 loopback，再让现有 Nginx 的 H5 子域反向代理到该端口；禁止用 `server_name _` 覆盖服务器上的其他站点。
