...
describe("admin content management", () => {
  ...
  it("rejects invalid internal identifiers, URLs, empty titles, and storage metadata", async () => {
    ...
    await expect(service.createAsset({ reportCardId: cardId || "x", title: "x", assetType: "IMAGE", openMode: "NEW_TAB", storageKey: "report.png", mimeType: "image/png", sortOrder: 0, status: "DRAFT" }, adminId)).rejects.toThrow(/当前页面/);
  });
