...
vi.mock("next/navigation", () => ({ redirect, useRouter: () => ({ push: vi.fn(), prefetch: vi.fn(), back: vi.fn(), replace: vi.fn(), refresh: vi.fn() }) }));
...
describe("brand guide", () => {
  ...
  it("renders the fixed brand guide at /go", () => {
    ...
    expect(screen.getByText("向上滑动，或点击滑动提示进入档案")).toBeInTheDocument();
  });
});
