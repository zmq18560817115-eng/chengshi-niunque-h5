...
GROUPS = {
    ...
    "review": ("3-报告检测页面-模块2", "报告页2完整图.jpg"),
...
def build_loading_motion(config):
    """Separate the supplied mascot from the stationary loading-page artwork."""
    group=SOURCES/"h5-加载页输出"
    base=rgba(group/"加载页.jpg")
    texture=rgba(group/"背景纹理.png")
    box=(408,1702,770,2095)
    base.paste(texture.crop(box),box[:2])
    # The source bar is 1314 x 94. Its visible left end in the reference is
    # x=344; the automated masked registration matched an occluded section.
    base.alpha_composite(rgba(group/"资源 4.png"),(344,1992))
    base.resize((750,1625),Image.Resampling.LANCZOS).save(OUTPUT/"runtime/loading-motion-base.webp","WEBP",quality=94,method=6)
    rgba(group/"资源 1.png").save(OUTPUT/"runtime/loading-label.webp","WEBP",lossless=True,method=6,exact=True)
    config["loadingMotion"]={"width":2000,"height":4333,"base":"/design/2026-09-07/runtime/loading-motion-base.webp","label":{"src":"/design/2026-09-07/runtime/loading-label.webp","x":416,"y":1710,"width":346,"height":377},"track":{"x":344,"y":1992,"width":1314,"height":94},"initialFill":418,"travel":896}


def build_editable_batch(config):
    """Remove only the three value fields in an opt-in editable variant.

    Fixed labels stay in the original artwork. The existing book underneath
    provides the same yellow texture. Default published values keep using the
    ...
    for x1,y1,x2,y2 in boxes:
        draw.rectangle((math.floor((x1-part["x"])*sx),math.floor((y1-part["y"])*sy),math.ceil((x2-part["x"])*sx),math.ceil((y2-part["y"])*sy)),fill=0)
    ...
    for x1,y1,x2,y2 in boxes:
        box=(math.floor(x1*scale),math.floor(y1*scale),math.ceil(x2*scale),math.ceil(y2*scale))
...
def build_review_card_copy(config, manifest):
    """Refresh only the two revised review-card parts at registered coordinates."""
    card = config["categories"]["review-assurance"]["cards"][2]
    card.update(title="过敏原项筛查", description="提前为宝宝排查150项致敏风险")
    for key, number in [("titleArtwork", 147), ("descriptionArtwork", 146)]:
        part = manifest["groups"]["review"]["parts"][f"资源 {number}"]
        artwork = rgba(SOURCES / part["source"]).resize((part["width"], part["height"]), Image.Resampling.LANCZOS)
        bounds = artwork.getchannel("A").getbbox()
        cropped = artwork.crop(bounds)
        width, height = cropped.size
        if width > 1000:
            cropped = cropped.resize((1000, round(height * 1000 / width)), Image.Resampling.LANCZOS)
        filename = f"review-{number}.webp"
        cropped.save(OUTPUT / "runtime" / filename, "WEBP", quality=92, method=6, exact=True)
        card[key] = {"src": f"/design/2026-09-07/runtime/{filename}",
                     "x": part["x"] + bounds[0], "y": part["y"] + bounds[1],
                     "width": width, "height": height}


def build_category_tails(config, manifest):
    """Fill tall-screen space with only the approved folder's bottom texture.

    The complete reference canvas and all its components keep their aspect
    ratios. This source crop is used only after that canvas ends.
    """
    for name,slug in [("inspection","inspection-projects"),("review","review-assurance"),("traceability","production-traceability")]:
        reference=rgba(SOURCES/manifest["groups"][name]["reference"])
        tail=reference.crop((0,3820,2000,4333)).resize((1000,257),Image.Resampling.LANCZOS)
        filename=f"{name}-page-tail.webp"
        tail.save(OUTPUT/"runtime"/filename,"WEBP",quality=92,method=6,exact=True)
        config["categories"][slug]["tail"]=f"/design/2026-09-07/runtime/{filename}"
...
def build():
    ...
    specs={
      ...
      "review": {"slug":"review-assurance","folder":107,"title":108,"footer":127,"cards":[[126,123,130,125,122,124,121],[120,117,129,119,116,118,109],[114,147,146,113,110,112,115]],"titles":["非必要物质实测","原料与工艺","过敏原项筛查"],"descriptions":["包含EPA、肉豆蔻酸实测含量，总糖实测含量，避免宝宝摄入过多的非必要脂肪酸和糖分，让每口都是需要的营养","包含FMT580藻油原料COA、原料溯源。妈妈可以看到原料来自哪里","提前为宝宝排查150项致敏风险"]},
    ...
    for name,spec in specs.items():
        ...
        for i,(blank,title,description,button,search,label,arrow) in enumerate(spec["cards"]):
            ...
            desc_y=title_y+135
            if description and not (name=="review" and description==146):fix(name,description,254,desc_y if name!="traceability" else 1077)
    ...
    build_category_tails(config,manifest)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--review-card-copy", action="store_true")
    ...
    if args.build: build()
    elif args.fish_motion or args.archive_entry or args.archive_module_one or args.review_card_copy or args.category_tails or args.editable_batch or args.loading_motion:
        ...
        elif args.review_card_copy: build_review_card_copy(config,manifest)
